...
sampler2D radiance_map : register(s1);
...
accum_pixel albedo_ps(
	s_simple_world_vertex IN) : COLOR
{
    float4 color;
...
accum_pixel static_default_ps(
	in s_simple_world_vertex fragment) : COLOR
{
	float4 albedo;
...
accum_pixel static_per_pixel_ps(
	...
	s_lightmap_per_pixel LIGHTMAP) : COLOR
{
    float4 out_color;
...
accum_pixel static_sh_ps(VS_OUTPUT_RAVI input) : COLOR
{
