...
PARAM_SAMPLER_2D(g_direction_lut);
...
#elif DX_VERSION == 11
    mask= sample2D(lightprobe_dir_and_bandwidth_ps, vTexCoord1).xyxy;
    tex[0]= lightprobe_hdr_color_ps.t.Sample(lightprobe_hdr_color_ps.s, float3(lightmap_texcoord_bottom.xy, 0));
    tex[1]= lightprobe_hdr_color_ps.t.Sample(lightprobe_hdr_color_ps.s, float3(lightmap_texcoord_bottom.xy, 1));
    tex[2]= lightprobe_hdr_color_ps.t.Sample(lightprobe_hdr_color_ps.s, float3(lightmap_texcoord_bottom.xy, 2));
#endif
...
#elif DX_VERSION == 11
    direction= g_direction_lut.t.Load(int3(dir_coord, 0)).wzyx;
#endif
...
#else
    uint2 seg01 = per_vertex_lightprobe_buffer.Load2(part1 * 4);
    seg0 = UnpackUByte4(seg01.x).wzyx;
#endif
...
#else
    seg1 = UnpackUByte4(seg01.y).wzyx;
#endif
