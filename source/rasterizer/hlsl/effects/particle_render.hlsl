...
extern float starting_uv_scale;
extern float ending_uv_scale;
extern float4 self_illum_color;
extern float bump_contrast;
extern float bump_randomness;
extern float contrast_scale;
extern float contrast_offset;

struct s_motion_blur_state
{
	float3 m_observer_velocity;
	float3 m_observer_rotation;
	float m_motion_blur_aspect_scale;
};
extern s_motion_blur_state g_motion_blur_state : register(c60);

struct s_sprite_definition
{
	float4 m_corner;
};
extern s_sprite_definition g_sprite : register(c63);
...
struct s_render_state
{
	...
	float m_animation_flags;
	float m_first_person;	// available for further runtime flags
	float m_curvature;
	float2 m_uv_scroll_rate;
	float m_game_time;
	float m_billboard_type;
	float m_vertex_count;
	s_fade m_fade;
	float4 custom_vertex_0; // pack verts 0,1 here
	float4 custom_vertex_1; // pack verts 1,2 here
};
extern s_render_state g_render_state : register(c70);
...
extern s_mesh_variant_list g_all_mesh_variants : register(c16);
...
extern s_sprite_frame_list g_all_sprite_frames;
...
s_particle_interpolators default_vs( vertex_type IN )
{
	...
	else
	{
		...
		// Hack alert.  The "deform" function is the only way to get a dependency on the vertex type.
		// We need to make the following be a compile-time branch; otherwise it impacts performance.
		bool IS_PARTICLE_MODEL= deform();
...
extern float3 screen_constants;
extern float distortion_scale;
...
extern float sphere_warp_scale;
...
float4 sample_diffuse(float2 texcoord_sprite, float2 texcoord_other_sprite, float2 texcoord_billboard, float palette_v, float particle_alpha, float depth_alpha)
{
	...
	{
		return tex2D(base_map, texcoord_sprite);
	}
	...
	{
		return float4(tex2D(base_map, texcoord_sprite).xyz, tex2D(alpha_map, texcoord_billboard).w);
	}
	
	IF_CATEGORY_OPTION(albedo, diffuse_plus_sprite_alpha)
	{
		return float4(tex2D(base_map, texcoord_sprite).xyz, tex2D(alpha_map, texcoord_sprite).w);
	}
	...
	{
		float index= tex2D(base_map, texcoord_sprite).x;
	...
	{
		float index= tex2D(base_map, texcoord_sprite).x;
		float alpha= tex2D(alpha_map, texcoord_billboard).w;
	...
	{
		float index= tex2D(base_map, texcoord_sprite).x;
		float alpha= tex2D(alpha_map, texcoord_sprite).w;
	...
	IF_CATEGORY_OPTION(albedo, palettized_plasma)
	{
		...
		float noise_b=	tex2D(base_map2,	texcoord_other_sprite * base_map2_xform.xy	+ base_map2_xform.zw).r;
...
#endif
		screen_coords *= depth_transform.xy;
...
s_particle_render_pixel_out default_ps(s_particle_interpolators INTERPOLATORS, float2 screen_coords : VPOS)
{
#ifndef pc
