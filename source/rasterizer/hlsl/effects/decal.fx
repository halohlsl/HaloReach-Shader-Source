...
PARAM(float, alpha_min);
PARAM(float, alpha_max);
...
float4 sample_diffuse(float2 texcoord_tile, float2 texcoord, float palette_v)
{
	...
	{
		return sample2D(base_map, transform_texcoord(texcoord, base_map_xform));
	}

	// Same as above except the alpha comes from a separate texture.
	IF_CATEGORY_OPTION(albedo, diffuse_plus_alpha)
	{
		return float4(sample2D(base_map, transform_texcoord(texcoord, base_map_xform)).xyz, sample2D(alpha_map, transform_texcoord(texcoord, alpha_map_xform)).w);
	}
	...
	{
		return float4(sample2D(base_map, transform_texcoord(texcoord, base_map_xform)).xyz, sample2D(alpha_map, transform_texcoord(texcoord_tile, alpha_map_xform)).w);
	}
	...
	{
		float index= sample2D(base_map, transform_texcoord(texcoord, base_map_xform)).x;
	...
	{
		float index= sample2D(base_map, transform_texcoord(texcoord, base_map_xform)).x;
		float alpha= sample2D(alpha_map, transform_texcoord(texcoord, base_map_xform)).w;
	...
	{
		float index= sample2D(base_map, transform_texcoord(texcoord, base_map_xform)).x;
		float alpha= sample2D(alpha_map, transform_texcoord(texcoord_tile, base_map_xform)).w;
		return float4(sample2D(palette, float2(index, palette_v)).xyz, alpha);
	}
