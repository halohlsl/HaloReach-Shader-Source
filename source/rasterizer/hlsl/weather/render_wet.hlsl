...
#include "weather\render_wet_registers.fx"
	...
	out float3 ripple_normal)
{
	...
	const float game_time= k_ps_ripple_scroll_coeff.x;
	{
		...
		float3 ripple_normal_0= sample3Dlod(k_ps_wet_sampler_ripple, ripple_texcoord, 1).xyz;
		...
		float3 ripple_normal_1= sample3Dlod(k_ps_wet_sampler_ripple, ripple_texcoord, 1).xyz;
	...
	out float3 reflection_color)
{
	...
	float4 reflection_0= sampleCUBElod(k_ps_wet_sampler_cubemap_0, reflect_dir, 0);
	float4 reflection_1= sampleCUBElod(k_ps_wet_sampler_cubemap_1, reflect_dir, 0);
