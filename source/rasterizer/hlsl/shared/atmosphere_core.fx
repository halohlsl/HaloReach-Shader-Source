...
{			
	...
	mixed_fog_color=  (fog_color*weight + base_fog_color*base_weight)/(weight + base_weight);
	...
	const bool fog_lights)
{
	...
	if (fog_enable)
	{
		...
		if (fog_lights)
		{
			...
			fog_light_scale= pow(fog_light_scale, fog_constants._fog_light_1_distance_falloff);
