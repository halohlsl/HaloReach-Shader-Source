...
#define raised_area_specular_offset 0.005f
	...
	float4	blend_target_0;
	float4	blend_target_1;
	float4	blend_target_2;
	float4	blend_target_3;
...
sampler base_map_m_##material_number;                    \
float4 base_map_m_##material_number##_xform;               \
sampler detail_map_m_##material_number;                    \
float4 detail_map_m_##material_number##_xform;                 \
sampler bump_map_m_##material_number;                    \
float4 bump_map_m_##material_number##_xform;               \
sampler detail_bump_m_##material_number;                  \
float4 detail_bump_m_##material_number##_xform;               \
float diffuse_coefficient_m_##material_number;               \
float specular_coefficient_m_##material_number;                 \
float specular_power_m_##material_number;                \
float3 specular_tint_m_##material_number;                  \
float fresnel_curve_steepness_m_##material_number;            \
float area_specular_contribution_m_##material_number;          \
float analytical_specular_contribution_m_##material_number;        \
float environment_specular_contribution_m_##material_number;       \
float albedo_specular_tint_blend_m_##material_number;
...
float4 sample_blend_normalized(float2 texcoord)
{
    ...
    float4 blend= tex2D(blend_map, transform_texcoord(texcoord, blend_map_xform));
...
void calc_bumpmap(
    ...
    out float2 bump)
{
    bump.xy= tex2D(bump_map, transform_texcoord(texcoord, bump_map_xform));

#if DETAIL_BUMP_ENABLED
    float2 detail= tex2D(detail_bump, transform_texcoord(texcoord, detail_bump_xform));
    bump.xy += detail.xy;
#endif 
}
...
{                                                                                \
    float4 base=      tex2D(base_map_m_##material,  transform_texcoord(original_texcoord, base_map_m_##material##_xform));     \
	BLEND_BASE(base, base_blend, material);																						\
    float4 detail=    tex2D(detail_map_m_##material, transform_texcoord(original_texcoord, detail_map_m_##material##_xform));    \
...
#ifdef pc
       diffuse_albedo= tex2D(albedo_texture, (fragment_position.xy + float2(0.5f, 0.5f)) / texture_size.xy);
#else     
       asm
       {
...
accum_pixel dynamic_light_ps(
    ...
    in float3 inscatter : COLOR1)       // homogenous coordinates of the fragment position in projective shadow space
{
    return default_dynamic_light_ps(fragment_position, texcoord, normal, binormal, tangent, fragment_to_camera_world, fragment_position_shadow, extinction, inscatter, false);
}
...
accum_pixel dynamic_light_cine_ps(
    ...
    in float3 inscatter : COLOR1)       // homogenous coordinates of the fragment position in projective shadow space
{
    return default_dynamic_light_ps(fragment_position, texcoord, normal, binormal, tangent, fragment_to_camera_world, fragment_position_shadow, extinction, inscatter, true);
}
