...
float4 sample_array_texture(in texture_sampler_2d_array array_texture, in float2 texcoord)
{
	...
	float4 lower=	sample2Dlod(array_texture, float4(tilecoord + mux_transform.xy, 0.0f, 0.0f));
	float4 higher=	sample2Dlod(array_texture, float4(tilecoord + mux_transform.zw, 0.0f, 0.0f));
