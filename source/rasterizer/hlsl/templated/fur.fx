...
sampler	fur_hairs_map;
float4	fur_hairs_map_xform;

float4	fur_deep_color;
sampler	fur_tint_map;
float4	fur_tint_map_xform;
float4	fur_tint_color;

float	layer_depth;
float	texcoord_aspect_ratio;			// how stretched your texcoords are
float	depth_darken;
int		layers_of_4;					// integer please
float	fur_intensity;
float	fur_alpha_scale;
float	fur_shear_x;						// read from warp map
float	fur_shear_y;						// read from warp map
float	fur_fix;
...
{
	float3	fur_color=		tex2D(fur_tint_map,	transform_texcoord(texcoord, fur_tint_map_xform));
...
//	[unroll]
	...
	{
		accum +=			depth_intensity * tex2D(fur_hairs_map,	texcoord);
		texcoord -=			offset;	
...
//	out float3 tangent						: TEXCOORD3,
	...
	out float3 fragment_to_camera_world		: TEXCOORD1)
{
	static_sh_vs(vertex, position, normal, fragment_to_camera_world);
}
...
//	in float3 tangent						: TEXCOORD3,
	...
	in float3 fragment_to_camera_world		: TEXCOORD1)
{
	return static_sh_ps(fragment_position, normal, fragment_to_camera_world);
}
