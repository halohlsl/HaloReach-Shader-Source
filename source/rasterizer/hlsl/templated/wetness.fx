...
PARAM(float, wet_material_dim_coefficient);
PARAM(float3, wet_material_dim_tint);
PARAM(float, wet_sheen_reflection_contribution);
PARAM(float3, wet_sheen_reflection_tint);
PARAM(float, wet_sheen_thickness);
PARAM(float, specular_mask_tweak_weight);
PARAM(float, surface_tilt_tweak_weight);


// ripple maps for rain drop ripples
PARAM_SAMPLER_2D(wet_flood_slope_map);
PARAM(float4, wet_flood_slope_map_xform);

PARAM_SAMPLER_2D(wet_noise_boundary_map);
PARAM(float4, wet_noise_boundary_map_xform);
...
#elif DX_VERSION == 11
			int wetness_values = int(k_vs_sampler_per_vertex_wetness.t.Load(int3(texcoord, 0)).x * 0xff);
			vertex_wetness = (wetness_values & (1 << int(channel_index))) ? 1.0f : 0.0f;
#endif
		}
	}
	...
	in float cubemap_blend_factor)
{
	...
	float4 reflection_0= sampleCUBE(dynamic_environment_map_0, reflect_dir);
	float4 reflection_1= sampleCUBE(dynamic_environment_map_1, reflect_dir);
#ifndef xenon
	// environment maps on xenon have an exponent adjustment of +2
	reflection_0 *= 4;
	reflection_1 *= 4;
   ...
   out float noised_per_pixel_reflection)
{
	...
	const float3 noise_value= sample2D(wet_noise_boundary_map, noise_texcoord);
	...
	in const float4 randomize_noise_direction)
{
	...
	float2 ripple_normal= sample3D(k_ps_sampler_wet_rain_ripple, float3(saturate(final_texcoord), layer_z)).xy;
	...
	inout float3 sheen_normal)
{
	...
	float3 ripple_normal_0= sample2Dlod(wet_flood_slope_map, ripple_texcoord, 0).xyz;
	...
	float3 ripple_normal_1= sample2Dlod(wet_flood_slope_map, ripple_texcoord, 0).xyz;
