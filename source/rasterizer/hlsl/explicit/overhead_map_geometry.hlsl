...
overhead_map_output default_vs(vertex_type IN)
{
    ...
    transformed_position.x= dot(float4(IN.position.xyz, 1.0f), map_transform1);
    transformed_position.y= dot(float4(IN.position.xyz, 1.0f), map_transform2);
    transformed_position.z= dot(float4(IN.position.xyz, 1.0f), map_transform3);

    OUT.HPosition= mul(float4(transformed_position, 1.0f), View_Projection);
    OUT.PosWorld= IN.position.xyz;
